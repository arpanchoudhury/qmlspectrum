...
#=== prediction
print('Out-of-sample confidence scores:')
if not os.path.isdir('prediction'):
    os.mkdir('prediction')

phi_query=[]
for iquery in range(N_mol-100,N_mol):

    Int_pred=qmlspectrum.predict(X,alpha,indices,iquery,max_size,sigmas,cut_distance)
    Int_TDDFT=Int_lam[iquery,:]

    f=open('prediction/Int'+str(iquery)+'.dat','w')
    ...
    phi_query.append(phi)
    print(iquery,phi)

phi_query=np.array(phi_query)
print('Mean confidence score is ',np.mean(phi_query))
